...
public class QuizManager {
    ...
    public void addNewQuiz() {
        ...
        while (true) {
            System.out.println("Please enter an option for the quiz (type 'done' when finished):");
    ...
    private void displayScoreAndFeedback(int score, int totalQuestion) {
        ...
        System.out.println("Quiz completed. Returning to main menu...\n");
    ...
    public void takeRandomQuiz() {
        ...
        System.out.println("How many question would you like to attempt? ");
        int numQuestions = scanner.nextInt();
        scanner.nextLine();

        // Limit the number of questions to the size of the list
        if(numQuestions > allQuestions.size()){
            System.out.println("Only " + allQuestions.size() + "questions are available.");
            numQuestions = allQuestions.size();
        }

        // Get the limited list of questions
        List<Question> limitedQuestions = allQuestions.subList(0, numQuestions);

        int score = askQuestions(limitedQuestions);
        int totalQuestions = limitedQuestions.size();
